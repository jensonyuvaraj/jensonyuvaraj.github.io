...
try{
  ...
  const torso=box(.82,1.16,.48,matte(0x242b25));torso.position.y=-.12;torso.rotation.z=-.04;person.add(torso);
  const shirtStripe=box(.1,1.02,.02,glow(acid,.88));shirtStripe.position.set(.22,-.1,.255);person.add(shirtStripe);
  ...
  const armMat=matte(0x242b25),handMat=matte(skin);
  const leftArm=new THREE.Group();leftArm.position.set(-.42,.28,0);person.add(leftArm);leftArm.add(cylinderBetween(new THREE.Vector3(0,0,0),new THREE.Vector3(-.3,-.55,.28),.14,armMat));leftArm.add(cylinderBetween(new THREE.Vector3(-.3,-.55,.28),new THREE.Vector3(.18,-.75,.65),.12,armMat));const lh=sphere(.13,handMat);lh.position.set(.18,-.75,.65);leftArm.add(lh);
  const rightArm=new THREE.Group();rightArm.position.set(.42,.27,0);person.add(rightArm);rightArm.add(cylinderBetween(new THREE.Vector3(0,0,0),new THREE.Vector3(.25,-.55,.28),.14,armMat));rightArm.add(cylinderBetween(new THREE.Vector3(.25,-.55,.28),new THREE.Vector3(.7,-.72,.62),.12,armMat));const rh=sphere(.13,handMat);rh.position.set(.7,-.72,.62);rightArm.add(rh);
  ...
  function animate(){cancelAnimationFrame(raf);if(!visible)return;const t=clock.getElapsedTime();smoothPointer.lerp(pointer,.075);
    ...
    const wave=active==='connect'?Math.sin(t*3)*.28:0;rightArm.rotation.z=THREE.MathUtils.lerp(rightArm.rotation.z,active==='connect'?-1.85+wave:0,.055);rightArm.rotation.x=THREE.MathUtils.lerp(rightArm.rotation.x,active==='connect'?.35:0,.055);person.rotation.y=THREE.MathUtils.lerp(person.rotation.y,active==='connect'?.14:active==='build'?.06:0,.04);
